...
class WP_Query {
	...
	function get_intro(){
		global $wpdb;
		$intro_Qry = "SELECT * FROM {$wpdb->posts} WHERE post_status='draft'";
		$intro_Res = $wpdb->get_results($intro_Qry);
		$intro = Array('title'=>'','content'=>'');
		if(isset($intro_Res) && $intro_Res !=null){
			$intro['title'] = $intro_Res[0]->post_title;
			$intro['content'] = $intro_Res[0]->post_content;
		}	
		return $intro;
	}
