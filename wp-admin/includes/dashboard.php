...
function wp_dashboard_setup() {
	...
	if ( is_blog_admin() && current_user_can( get_post_type_object( 'post' )->cap->create_posts ) ) {
		$quick_draft_title = sprintf( '<span class="hide-if-no-js">%1$s</span> <span class="hide-if-js">%2$s</span>', __( 'Introduction' ), ' ' );
...
function wp_dashboard_quick_press( $error_msg = false ) {
	...
	$p_id = $p_content = $p_title = '';
		...
		if ($drafts ) {
			$p_content = $drafts[0]->post_content;
			$p_title = $drafts[0]->post_title;
			$p_id = $drafts[0]->ID;
	...
	<form name="post" action="<?php echo esc_url( admin_url( 'post.php' ) ); ?>" method="post" id="quick-press" class="initial-form hide-if-no-js">
		...
		<div class="input-text-wrap" id="title-wrap">
			...
			<input type="text" name="post_title" id="title" value="<?=$p_title?>" autocomplete="off" />
		</div>

		<div class="textarea-wrap" id="description-wrap">
			<label class="screen-reader-text prompt" for="content" id="content-prompt-text"><?php _e( 'What&#8217;s on your mind?' ); ?></label>
			<textarea name="content" id="content" class="mceEditor" rows="3" cols="15" autocomplete="off"><?php echo $p_content;?></textarea>
		</div>

		<p class="submit">
			<?php  if($p_id!=''){ ?>
				<input type="hidden" name="action" id="quickpost-action" value="post-quickdraft-update" />
